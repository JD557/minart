...
class AxisAlignedBoundingBoxSpec extends munit.FunSuite {
  ...
  test("Merge two bounding boxes when there's a gap") {
    ...
    assertEquals(aabb1 ++ aabb2, AxisAlignedBoundingBox(10, 10, 30, 10))
    assertEquals(aabb2 ++ aabb1, AxisAlignedBoundingBox(10, 10, 30, 10))
  }

  test("Merge two bounding boxes when they intersect") {
    val aabb1 = AxisAlignedBoundingBox(10, 10, 10, 10)
    val aabb2 = AxisAlignedBoundingBox(15, 10, 10, 10)
    assertEquals(aabb1 ++ aabb2, AxisAlignedBoundingBox(10, 10, 15, 10))
    assertEquals(aabb2 ++ aabb1, AxisAlignedBoundingBox(10, 10, 15, 10))
  }

  test("Return an empty bounding box intersection when there's a gap") {
    val aabb1 = AxisAlignedBoundingBox(10, 10, 10, 10)
    val aabb2 = AxisAlignedBoundingBox(30, 10, 10, 10)
    assertEquals((aabb1 & aabb2).isEmpty, true)
    assertEquals((aabb2 & aabb1).isEmpty, true)
  }

  test("Shrink two bounding boxes when they intersect") {
    val aabb1 = AxisAlignedBoundingBox(10, 10, 10, 10)
    val aabb2 = AxisAlignedBoundingBox(15, 10, 10, 10)
    assertEquals(aabb1 & aabb2, AxisAlignedBoundingBox(15, 10, 5, 10))
    assertEquals(aabb2 & aabb1, AxisAlignedBoundingBox(15, 10, 5, 10))
  }
}
